...
enum planck_layers { _QWERTY, _COLEMAK, _DVORAK, _LOWER, _RAISE, _PLOVER, _ADJUST, _MEDIA };

enum planck_keycodes { QWERTY = SAFE_RANGE, COLEMAK, DVORAK, PLOVER, BACKLIT, EXT_PLV, SECRET };
 ...
 * |------+------+------+------+------+------+------+------+------+------+------+------|
 * | Media| Ctrl | Alt  | GUI  |Lower |    Space    |Raise | Left | Down |  Up  |Right |
 * `-----------------------------------------------------------------------------------'
 */
[_COLEMAK] = LAYOUT_planck_grid(
    ...
    MEDIA,   KC_LCTL, KC_LALT, KC_LGUI, LOWER, KC_RSFT, KC_SPC, RAISE, KC_LEFT, KC_DOWN, KC_UP,   KC_RGHT
),
...
[_LOWER] = LAYOUT_planck_grid(
    KC_TILD, KC_EXLM, KC_AT,   KC_HASH, KC_DLR,  KC_PERC, KC_CIRC, KC_AMPR, KC_UNDS, _______, _______, KC_DEL,
...
/* Adjust (Raise + Lower)
* ,-----------------------------------------------------------------------------------.
* |Flash |      |Qwerty|      |AudTgl|      |      |      |RGBTgl|RGBMod|      |      |
* |------+------+------+------+------+-------------+------+------+------+------+------|
* |Debug |      |Plover|      |MusTgl|      |      |      |RGBHu-|RGBHu+|      |      |
* |------+------+------+------+------+------|------+------+------+------+------+------|
* |EEClr |      |Dvorak|      |      |      |      |      |RGBSa-|RGBSa+|      |      |
* |------+------+------+------+------+------+------+------+------+------+------+------|
* |      |      |Colemk|      |      |             |      |RGBBr-|RGBBr+|      |      |
* `-----------------------------------------------------------------------------------'
*/
    ...
    switch (keycode) {
        case SECRET:
            if (record->event.pressed) {
                ...
                SEND_STRING_DELAY("Secret", 20);
            }
            return false;
            break;
        case QWERTY:
            if (record->event.pressed) {
                println("Switching mode to QWERTY");
        ...
        case COLEMAK:
            if (record->event.pressed) {
                println("Switching mode to Colemak");
        ...
        case DVORAK:
            if (record->event.pressed) {
                println("Switching mode to Dvorak");
        ...
        case PLOVER:
            if (record->event.pressed) {
                println("Switching mode to PLOVER");
#ifdef AUDIO_ENABLE
                stop_all_notes();
